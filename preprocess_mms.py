...
for part_ in ["train", "dev"]:
    ...
    target_dir = os.path.join("D:/mywork/preprocess_xls-r-5_wav2vec2-large-xlsr-53-chinese-zh-cn", part_,
                              "xls-r-5")
    config = Wav2Vec2Config.from_json_file(r"D:\下载\codecfake_data\wav2vec2-large-xlsr-53-chinese-zh-cn/config.json")
    processor = Wav2Vec2FeatureExtractor.from_pretrained(r"D:\下载\codecfake_data\wav2vec2-large-xlsr-53-chinese-zh-cn")
    model = Wav2Vec2Model.from_pretrained(r"D:\下载\codecfake_data\wav2vec2-large-xlsr-53-chinese-zh-cn").cuda()
    #processor =  Wav2Vec2FeatureExtractor.from_pretrained("facebook/wav2vec2-xls-r-300m")
    model.config.output_hidden_states = True

    if not os.path.exists(target_dir):
        os.makedirs(target_dir)
    #LL = nn.Linear(768, 1024)
    for idx in tqdm(range(len(codecspoof_raw))):
        ...
        with torch.no_grad():
            ...
            #wav2vec2 = LL(wav2vec2)
        # # torch.Size([1, 201, 1024])
        print(wav2vec2.shape)
        print(wav2vec2.device)
        torch.save(wav2vec2.float(), os.path.join(target_dir, "%s_%06d_%s_%s.pt" % ("chinese-wav2vec2-base",idx, os.path.splitext(os.path.basename(filename))[0], label.strip())))
    print("Done")
...
# for part in ["train","dev"]:
#     for part_ in ["0.txt"]:
#         codecspoof_raw = dataset.data_real_fake(f"D:/mywork/", part=part+"_"+part_)
#         target_dir = os.path.join("D:/mywork/preprocess_xls-r-5_add_wavefake", part,
#                                   "xls-r-5")
#         config = Wav2Vec2Config.from_json_file("huggingface/wav2vec2-xls-r-300m/config.json")
#         processor = Wav2Vec2FeatureExtractor.from_pretrained("huggingface/wav2vec2-xls-r-300m/")
#         model = Wav2Vec2Model.from_pretrained("huggingface/wav2vec2-xls-r-300m/").cuda()
