...
class codecfake(Dataset):
    ...
    def collate_fn(self, samples):
        return default_collate(samples)
class finvcup9thfake(Dataset):
    def __init__(self, path_to_database, part='train'):
        super(finvcup9thfake, self).__init__()
        self.ptd = path_to_database
        protocol = path_to_database +"/finvcup9th_1st_ds5_"+ part +"_data.csv"
        #finvcup9th_1st_ds5_train_data.csv
        self.label = {"fake": 0, "real": 1}
        with open(protocol, 'r') as f:
            csvheader = csv.reader(f)
            next(csvheader)
            audio_info = [info for info in csvheader]
            self.all_info = audio_info
...
class data_real_fake(Dataset):
    def __init__(self, path_to_database, part='train.txt'):
        super(data_real_fake, self).__init__()
        self.ptd = path_to_database
        protocol = path_to_database + part
        #finvcup9th_1st_ds5_train_data.csv
        with open(protocol, 'r') as f:
            #audio_info = [[info.strip(),"0"]if part == "0.txt" else [info.strip(),"1"] for info in f.readlines()]
            audio_info = [[info.strip(), "0"] for info in f.readlines()]
        self.all_info = audio_info
